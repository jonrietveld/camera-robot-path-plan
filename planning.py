...
def findPointOnPathAndSlope(pathArr,time,config = None):
	#find closest point in time on a given path below the given time through linear interpolation
	closeTime = pathArr.index(min(pathArr,key=lambda x: abs(x[-1]-time)))   
	if 0 < time < pathArr[closeTime][-1]:
		closeTime = closeTime - 1
	if closeTime == len(pathArr) - 1:											# If the closest time index is the last index of our pathArr, then decrement closeTime by one for slope calculation in next step.
		closeTime -= 1
	
	slope = [pathArr[closeTime+1][0]-pathArr[closeTime][0], pathArr[closeTime+1][1]-pathArr[closeTime][1]]	# Find slope between current point and the point following it in the pathArr
	if slope == [0,0]:															# If we have zero slope, to avoid errors in calculation, set slope to [1,0]
		slope = [1,0]
	else:
		slope = np.array(slope)/(np.array(slope)**2).sum()**.5					#Convert slope to unit vector
	
	if time - pathArr[closeTime][-1] < 0.001: 									# Due to python rounding errors, if our given time is close to pathArr[closeTime] continue into inner if statement 
		if len(pathArr[0]) == 5:												# If requested time is in array, just return that point 
...
def solve(inputconfig,VERBOSE = False):
	...
	def findPathToShot(current_position,shot_start_loc,robot_cfg,parent_node,shot):
		...
		#Check to see if direct dubins path passes over obsticles in environment png file
		error_test = 0
	...
	def isFollowableShot(actor_path_followable, robot_followable,shot_followable):
		...
		# Finally evaluate equation to check if the shot is followable by the robot with radius of curvature equation found in research paper.
		for time_actor in range(len(qx)):
	...
	def finish(current_node):
		# Called when all paths are solved as the final step before returning the resulting configuration to the calling program.
		robot_paths = []
