...
def solve(inputconfig):
	...
	def binarySearch(data, val):
		...
		while highIndex > lowIndex:
				...
				sub = data[index].f
				if data[lowIndex].f == val:
						return [lowIndex, lowIndex]
				elif sub == val:
						return [index, index]
				elif data[highIndex].f == val:
	...
	def findPossiblePositions(node,solved_config,maze): 
		...
		for xpoint in np.linspace(-1.8,1.8,1.8/solved_config['solve']['map_resolution'],endpoint=False):
			for ypoint in np.linspace(-1.8,1.8,1.8/solved_config['solve']['map_resolution'],endpoint=False):
				#print(xpoint,ypoint)
	...
	def hueristic(node,solved_config,end_time): #dist from actor
		...
		return dist_from_actor*(solved_config['solve']['time_resolution']) + time_to_end
	...
	def lossFunction(node,solved_config):
		not_catching_shot_loss = 500

		loss = 1*(solved_config['solve']['time_resolution'])/2 #loss to move 1 positon
		...
		for shot in solved_config['shots']:
			if shot['start_time'] <= node.position[3] <= shot['end_time']:
				...
				if insidePoly:
					...
					dist_from_actor = hypot(center_polygon[0] - actor_xy[0], center_polygon[1] - actor_xy[1])
					#print(loss)
					loss += dist_from_actor*(1/512)
	...
	def astar(maze, start, solved_config):
		...
		while len(open_list) > 0:
			...
			print("x = {:<20}y = {:<20}t = {:<20}g = {:<20}h = {:<20}f = {:<20}".format(current_node.position[0],current_node.position[1],current_node.position[3], current_node.g, current_node.h,current_node.f))
			...
			if current_node.position[3] >= 9.0: #end_time:	#changed
				path = []
			...
			for child in children:
				...
				child.g = round(current_node.g + lossFunction(child,solved_config),10)									#changed
				child.h = round(hueristic(child,solved_config,end_time),10)												#changed
...
#visualize(config)
solved_config = yaml.safe_load(open('result.yaml','r'))
